...
def get_currently_modified_folders():
    """Detects newly added or modified problem folders in git workspace."""
    try:
        status_output = subprocess.check_output(
            ["git", "status", "--porcelain"], text=True, errors="ignore"
        )
...
def parse_existing_archive(readme_content):
    """Parses existing topic sections and problem folders from README.md."""
    pattern = r"<!---LeetCode Topics Start-->(.*?)<!---LeetCode Topics End-->"
    ...
    for line in block.splitlines():
        ...
        if current_topic and line.startswith("|") and not line.startswith("| Problem") and not line.startswith("| :---"):
            problem_match = re.search(r"\[([^\]]+)\]", line)
            if problem_match:
                existing_map[current_topic].add(problem_match.group(1).strip())
...
def main():
    ...
    if os.path.exists(README_PATH):
        ...
        topic_map = parse_existing_archive(existing_content)
    ...
    for folder in all_folders:
        # Standardize slug: 0206-Reverse_Linked_List -> reverse-linked-list
        raw_slug = re.sub(r"^\d+-", "", folder).strip().lower()
        slug = re.sub(r"[\s_]+", "-", raw_slug)

        cached_tags = cache.get(folder, [])

        # Fetch tags if changed, uncached, or invalid
        if folder in changed_folders or not cached_tags or cached_tags == ["Uncategorized"]:
            print(f"🔍 Classifying tags for: {folder} (slug: {slug})")
            tags = fetch_leetcode_topics_api(slug)
    ...
    # Build updated Archive Markdown Block
    lines = ["<!---LeetCode Topics Start-->", "# My Leet_Code Archive\n"]
    for topic in sorted(topic_map.keys()):
        lines.append(f"## {topic}\n")
        lines.append("| Problem |")
    ...
    # Insert or update in README.md
    pattern = r"<!---LeetCode Topics Start-->.*?<!---LeetCode Topics End-->"
    if re.search(pattern, existing_content, re.DOTALL):
        new_content = re.sub(pattern, archive_block, existing_content, flags=re.DOTALL)
    elif existing_content:
        new_content = existing_content.rstrip() + "\n\n" + archive_block
    else:
        new_content = archive_block

    with open(README_PATH, "w", encoding="utf-8") as f:
        f.write(new_content)
        
    print("\n🎉 README.md archive section updated and merged successfully!")
