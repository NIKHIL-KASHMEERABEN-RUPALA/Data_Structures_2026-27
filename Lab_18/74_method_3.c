...
// Min Priority Queue ma element insert karva mate nu function (Ascending Order)
void enqueue(int value)
{
    ...
    // Min Priority Logic: Shift larger elements to the right to place smaller value at the front
    int i = size - 1;
    while (i >= 0 && pq[i] > value)
    {
        pq[i + 1] = pq[i]; // Mota elements ne right side shift karo
        i--;
    }
    ...
    printf("%d inserted successfully into Min Priority Queue.\n", value);
}

// Highest Priority Element (Minimum Value) delete karva mate nu dequeue function
void dequeue()
{
    ...
    // Array ascending order ma hovathi index 0 par hammesha minimum element (highest priority) j hase
    printf("Deleted Min Priority Element: %d\n", pq[0]);
